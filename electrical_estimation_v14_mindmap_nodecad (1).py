...
APP_TITLE = "ELECTimate AI v13.6 ToDoマインドマップAI + ノード図面作成版"
...
def scan_color_points(small_img, rule, settings, step):
    """
    画像から色ルールに一致する座標点群を抽出する。
    numpyが使えるときはベクトル演算で一括判定（巨大画像でもフリーズしない）。
    numpyが無い環境ではPythonループにフォールバックする。
    """
    sw, sh = small_img.size
    np = try_import_numpy()
    if np is not None:
        arr = np.asarray(small_img.convert('RGB'))
        sub = arr[0:sh:step, 0:sw:step, :]
        r = sub[:, :, 0].astype(np.int32); g = sub[:, :, 1].astype(np.int32); b = sub[:, :, 2].astype(np.int32)
        mask = (
            (r >= int(rule['r_min'])) & (r <= int(rule['r_max'])) &
            (g >= int(rule['g_min'])) & (g <= int(rule['g_max'])) &
            (b >= int(rule['b_min'])) & (b <= int(rule['b_max']))
        )
        if settings.get('exclude_legend', '1') == '1':
            rr = float(settings.get('legend_right_ratio', '0.24'))
            br = float(settings.get('legend_bottom_ratio', '0.28'))
            ys_idx = np.arange(0, sh, step)
            xs_idx = np.arange(0, sw, step)
            legend_x = xs_idx >= int(sw * (1 - rr))
            legend_y = ys_idx >= int(sh * (1 - br))
            legend_mask = legend_y[:, None] & legend_x[None, :]
            mask &= ~legend_mask
        ys, xs = np.nonzero(mask)
        pts = list(zip((xs * step).tolist(), (ys * step).tolist()))
        return pts
    px = small_img.load(); pts = []
...
def estimate_unit_cell_size(clusters, default_wh=(20, 20)):
    """同一ルールの各クラスタサイズの中央値から「単一記号1個分」の標準サイズを推定する。"""
    sizes = []
    for cl in clusters:
        ...
        sizes.append((w, h))
    if not sizes: return default_wh
    ws = sorted(s[0] for s in sizes); hs = sorted(s[1] for s in sizes)
    n = len(ws)
    med_w = ws[n // 2]; med_h = hs[n // 2]
    keep = [(w, h) for w, h in sizes if 0.4 * med_w <= w <= 1.8 * med_w and 0.4 * med_h <= h <= 1.8 * med_h]
    if keep:
        med_w = sorted(w for w, h in keep)[len(keep) // 2]
        med_h = sorted(h for w, h in keep)[len(keep) // 2]
    return max(3, med_w), max(3, med_h)

def _split_axis_by_density(vals, unit_len, min_len):
    """
    1軸上の点座標列を「記号1個ぶんの中心」ごとに分割する。
    重なっている記号どうしは点密度がゼロまで落ちる隙間ができないことが多いため、
    谷ではなく密度の「山（ピーク＝記号の中心）」を検出して分割する方式にしている。
    """
    if not vals: return [(0, 0)]
    lo, hi = min(vals), max(vals)
    span = hi - lo + 1
    if span <= unit_len * 1.5:
        return [(lo, hi)]
    bins = max(1, int(round(span)))
    hist = [0] * bins
    for v in vals: hist[min(bins - 1, v - lo)] += 1
    # 平滑化（ノイズによる偽ピーク防止）
    smooth = hist[:]
    for _ in range(2):
        prev = smooth[:]
        for i in range(1, bins - 1):
            smooth[i] = (prev[i - 1] + prev[i] * 2 + prev[i + 1]) / 4.0
    min_dist = max(3, int(unit_len * 0.55))
    peaks = []
    for i in range(1, bins - 1):
        if smooth[i] >= smooth[i - 1] and smooth[i] >= smooth[i + 1] and smooth[i] > 0:
            if not peaks or i - peaks[-1] >= min_dist:
                ...
                peaks[-1] = i
    expected_n = max(1, round(span / max(1.0, unit_len)))
    if len(peaks) <= 1 or expected_n <= 1:
        return [(lo, hi)]
    # ピークとピークの中間点を境界にして分割する
    bounds = [0]
    for a, b in zip(peaks[:-1], peaks[1:]):
        bounds.append((a + b) // 2)
    bounds.append(bins - 1)
    segments = []
    for i in range(len(bounds) - 1):
        s0, s1 = bounds[i], bounds[i + 1]
        if s1 <= s0: continue
        segments.append((s0 + lo, s1 + lo))
    merged = []
    for seg in segments:
        if merged and (seg[1] - seg[0] + 1) < min_len:
            ms, me = merged[-1]; merged[-1] = (ms, seg[1])
        else:
            merged.append(seg)
    return merged if merged else [(lo, hi)]

def split_cluster_by_projection(cl, unit_w, unit_h, min_pts=3):
    """
    重なり・密集・隣接した記号の点群クラスタを、X/Y軸の密度投影の「谷」から
    個々の記号らしき部分集合へ分離する（追加ライブラリ不要の軽量ロジック）。
    """
    xs = [p[0] for p in cl]; ys = [p[1] for p in cl]
    bw = max(xs) - min(xs) + 1; bh = max(ys) - min(ys) + 1
    if bw <= unit_w * 1.7 and bh <= unit_h * 1.7:
        return [cl]
    x_segs = _split_axis_by_density(xs, unit_w, max(3, int(unit_w * 0.5)))
    y_segs = _split_axis_by_density(ys, unit_h, max(3, int(unit_h * 0.5)))
    if len(x_segs) <= 1 and len(y_segs) <= 1:
        return [cl]
    out = []
    for (x0, x1) in x_segs:
        for (y0, y1) in y_segs:
            sub = [p for p in cl if x0 <= p[0] <= x1 and y0 <= p[1] <= y1]
            if len(sub) >= min_pts:
                out.append(sub)
    if not out or len(out) > 12 or all(len(s) < min_pts * 1.5 for s in out):
        return [cl]
    return out
...
def split_large_detection_smart(d, img, rules, settings, exp_w=55, exp_h=55, force=False):
    """
    大型マージ検出を「実際の記号位置」で分割する。
    均等グリッドではなく、切り出した領域内の色点群を密度ピーク（記号中心）で
    分割することで、重なり/密集/隣接した記号を個々の記号候補へ分離する。
    exp_w/exp_hは「記号1個ぶんの想定サイズ」（呼び出し元の設定 max_box_w/h 等）。
    切り出した領域だけからでは単体サイズを正しく推定できないため、必ず外から渡す。
    force=Trueの場合は面積が小さくても（ユーザーが手動で「分離」を指示した場合）分割を試みる。
    """
    bw=d.x2-d.x1; bh=d.y2-d.y1
    if bw*bh<100 and not force: return [d]
    ...
    step=max(1,int(settings.get('scan_step','4')))
    min_pts=max(3,int(settings.get('min_cluster_points','3')))
    cluster_dist=max(3,int(settings.get('cluster_distance','6')))

    if matching_rule:
        sub_settings=dict(settings); sub_settings['exclude_legend']='0'
        pts=scan_color_points(crop, matching_rule, sub_settings, step)
    elif force:
        # 手動分離要求時はルール未特定でも「白ではない画素」を対象に分離を試みる
        px=crop.load(); cw,ch=crop.size; pts=[]
        for y in range(0,ch,step):
            for x in range(0,cw,step):
                r,g,b=px[x,y]
                if r<245 or g<245 or b<245: pts.append((x,y))
    else:
        d.enabled=False; d.memo=(d.memo+' / large box disabled (no matching rule)').strip()
        return [d]

    clusters=cluster_points(pts,cluster_dist)
    uw,uh=max(6,int(exp_w)),max(6,int(exp_h))
    result=[]
    for cl in clusters:
        for sub in split_cluster_by_projection(cl,uw,uh,min_pts=min_pts):
            if len(sub)<min_pts: continue
            xs=[p[0] for p in sub]; ys=[p[1] for p in sub]
            result.append(ImageDetection(
                0,d.page,cx1+min(xs),cy1+min(ys),cx1+max(xs),cy1+max(ys),
                d.color_name,d.equipment,
                min(0.99,max(0.1,len(sub)/max(1,(max(xs)-min(xs)+1)*(max(ys)-min(ys)+1)))),len(sub),
                True,'smart_split' if matching_rule else 'manual_separate',
                f'smart_split from {bw}x{bh}'
            ))
    if len(result)>=2:
        return result
    if force:
...
def analyze_image_page_light(img,page_no,settings,rules,log=None):
    ...
    auto_separate = settings.get('auto_separate_symbols','1') == '1'
    small,ratio=shrink_image(img,max_w); sw,sh=small.size; dets=[]; split_dets=[]
    for rule in rules:
        if int(rule.get('enabled',1))!=1: continue
        pts=scan_color_points(small,rule,settings,step)
        if log: log(f"page {page_no}: {rule['color_name']} sampled_points={len(pts)}")
        raw_clusters=cluster_points(pts,dist)
        # 重なり・密集・隣接した記号どうしを分離するため、想定単体サイズより
        # 明らかに大きい塊は密度投影のピーク（記号中心）を使って個々の記号候補へ分割する。
        unit_w,unit_h=estimate_unit_cell_size(raw_clusters, default_wh=((min_w+max_bw)//2 or 20,(min_h+max_bh)//2 or 20))
        clusters=[]; is_split_origin=[]
        for cl in raw_clusters:
            if auto_separate and len(cl)>=min_pts*2:
                sub=split_cluster_by_projection(cl,unit_w,unit_h,min_pts=min_pts)
            else:
                sub=[cl]
            was_split = len(sub) > 1
            for s in sub:
                clusters.append(s); is_split_origin.append(was_split)
        for cl,was_split in zip(clusters,is_split_origin):
            ...
            inv=1/ratio
            # 分離済みクラスタは余白(pad)を最小限にする。通常のpadを付けると
            # 隣り合う分離済みボックス同士が再び接触/重複してしまい、後段のmerge_detsで
            # せっかく分離した記号が再結合してしまうため。
            pad=int((1 if was_split else 5)*inv)
            x1=max(0,int(sx1*inv)-pad); y1=max(0,int(sy1*inv)-pad); x2=min(img.width-1,int(sx2*inv)+pad); y2=min(img.height-1,int(sy2*inv)+pad)
            det=ImageDetection(0,page_no,x1,y1,x2,y2,rule['color_name'],rule['equipment'],min(0.99,max(0.1,len(cl)/max(1,bw*bh))),len(cl))
            if was_split:
                det.source='auto_split'; det.memo='auto_separated'
                split_dets.append(det)
            else:
                dets.append(det)
    img_max_bw=int(max_bw/max(0.1,ratio)); img_max_bh=int(max_bh/max(0.1,ratio))
    merge_dist = int(dist/max(0.1,ratio))
    dets=merge_dets(dets,merge_dist,max_merged_w=img_max_bw,max_merged_h=img_max_bh)
    # auto_separateで分離済みの候補は、せっかく分離した記号が再結合しないよう
    # 通常の近接統合(merge_dets)にはかけずそのまま採用する。
    dets=dets+split_dets
    dets=sorted(dets,key=lambda d:d.pixel_count,reverse=True)[:max_dets]; dets=sorted(dets,key=lambda d:(d.page,d.y1,d.x1))
    for idx,d in enumerate(dets,1): d.det_id=idx
    if log: log(f'page {page_no}: 検出候補 {len(dets)}件（うち自動分離 {len(split_dets)}件）')
    return dets
...
class NodePropertyDialog(tk.Toplevel):
    def __init__(self, parent, node=None):
        ...
        for i,(lab,var) in enumerate(rows):
            ttk.Label(f,text=lab).grid(row=i,column=0,sticky='e',pady=4)
            if lab=='ノード種別':
                ttk.Combobox(f,textvariable=var,values=['material','tool','step','category','symbol','cable','panel','switch','outlet','light','process'],width=24).grid(row=i,column=1,sticky='w')
            elif lab=='図記号':
...
class IntegratedApp:
    def __init__(self, root):
        ...
        self.zoom_min=0.03; self.zoom_max=12.0; self._render_after_id=None
        ...
        self.ui_q=queue.Queue()  # v14: ワーカースレッド→メインスレッドのUI更新もキュー経由に統一
        self.init_optional_ui_state(); self.build_ui(); self.refresh_all()
        self.root.after(100, self._drain_log_q)
        self.root.after(30, self._drain_ui_q)
    ...
    def ui(self, fn, *args, **kwargs):
        """
        ワーカースレッドからメインスレッドのUIを安全に更新する。
        以前は別スレッドから直接 root.after(0, ...) を呼んでいたが、
        Spyder/Jupyter等、Tkinterのmainloopをアプリ側で直接ブロッキング実行しない
        環境では「main thread is not in main loop」で更新が失敗し、
        画面に何も表示されない（＝図面が読み込まれていないように見える）
        症状が起きることが判明した。
        キューに積み、メインスレッド自身が定期的に(_drain_ui_q)処理することで、
        root.after()を呼ぶのは常にメインスレッド自身にする。
        """
        self.ui_q.put((fn, args, kwargs))

    def _drain_ui_q(self):
        try:
            while True:
                fn, args, kwargs = self.ui_q.get_nowait()
                try:
                    fn(*args, **kwargs)
                except Exception:
    ...
    def build_image_tab(self):
        f=self.tab_img; top=ttk.Frame(f); top.pack(fill=tk.X,padx=2,pady=1); self.image_file_var=tk.StringVar(); ttk.Button(top,text='操作▼',width=7,command=self.show_image_command_menu).pack(side=tk.LEFT,padx=1); ttk.Entry(top,textvariable=self.image_file_var).pack(side=tk.LEFT,fill=tk.X,expand=True,padx=2); ttk.Button(top,text='📂',command=self.select_image_file).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='解析',command=self.start_image_analysis).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='最大',command=self.maximize_drawing_area).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='復帰',command=self.restore_drawing_area).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='CSV',command=self.export_image_csv).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='編集',command=self.edit_selected_detection).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='削除',command=self.delete_selected_detection).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='分離',command=self.separate_selected_detection).pack(side=tk.LEFT,padx=2); ttk.Checkbutton(top,text='線ON',variable=self.cable_draw_mode_var).pack(side=tk.LEFT,padx=2); ttk.Checkbutton(top,text='スナップON',variable=self.snap_enabled_var).pack(side=tk.LEFT,padx=2); ttk.Entry(top,textvariable=self.snap_grid_px_var,width=4).pack(side=tk.LEFT,padx=1); ttk.Button(top,text='線編集',command=self.edit_selected_cable).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='線削除',command=self.delete_selected_cable).pack(side=tk.LEFT,padx=2); ttk.Button(top,text='記号DB',command=self.register_symbol_image_dataset).pack(side=tk.LEFT,padx=2); ttk.Combobox(top,textvariable=self.symbol_paste_var,values=['LEDダウンライト','LEDベースライト','コンセント','片切スイッチ','分電盤','換気扇','非常灯','誘導灯','未分類設備'],width=14).pack(side=tk.LEFT,padx=2); ttk.Checkbutton(top,text='記号貼付ON',variable=self.symbol_paste_mode_var).pack(side=tk.LEFT,padx=2)
        ...
        zoombar=ttk.Frame(preview_frame); zoombar.pack(fill=tk.X)
        ttk.Button(zoombar,text='－',width=3,command=lambda:self.zoom_step(1/1.25)).pack(side=tk.LEFT,padx=1)
        ttk.Button(zoombar,text='＋',width=3,command=lambda:self.zoom_step(1.25)).pack(side=tk.LEFT,padx=1)
        ttk.Button(zoombar,text='フィット',width=6,command=self.zoom_fit).pack(side=tk.LEFT,padx=1)
        ttk.Button(zoombar,text='100%',width=5,command=self.zoom_100).pack(side=tk.LEFT,padx=1)
        self.zoom_label_var=tk.StringVar(value='100%')
        ttk.Label(zoombar,textvariable=self.zoom_label_var,width=6).pack(side=tk.LEFT,padx=4)
        ttk.Label(zoombar,text='ホイール=拡大縮小 / Shift+ホイール=左右 / Ctrl+ホイール=上下').pack(side=tk.LEFT,padx=8)
        canvas_area=ttk.Frame(preview_frame); canvas_area.pack(fill=tk.BOTH,expand=True)
        self.preview_canvas=tk.Canvas(canvas_area,bg='white',width=1120,height=780)
        self.preview_vbar=ttk.Scrollbar(canvas_area,orient=tk.VERTICAL,command=self._preview_yview)
        self.preview_hbar=ttk.Scrollbar(canvas_area,orient=tk.HORIZONTAL,command=self._preview_xview)
        self.preview_canvas.configure(yscrollcommand=self._preview_vbar_set,xscrollcommand=self._preview_hbar_set)
        ...
        # Linux(X11)ではホイールがButton-4/5として届く環境がある
        ...
        self.preview_canvas.bind('<Configure>',lambda e:self._schedule_preview_render())
    ...
    def _preview_yview(self,*args):
        try: self.preview_canvas.yview(*args)
        except Exception: pass
        self._schedule_preview_render()
    def _preview_xview(self,*args):
        try: self.preview_canvas.xview(*args)
        except Exception: pass
        self._schedule_preview_render()
    def _preview_vbar_set(self,*args):
        try: self.preview_vbar.set(*args)
        except Exception: pass
        self._schedule_preview_render()
    def _preview_hbar_set(self,*args):
        try: self.preview_hbar.set(*args)
        except Exception: pass
        self._schedule_preview_render()

    def _preview_canvas_size(self):
        """
        winfo_width()/height()はウィジェットがまだ画面に配置されていない時
        既定値の1を返すことがあり、`x or fallback`では1が真値扱いされるため
        フォールバックが効かない。ここで明示的に閾値判定して安全な値を返す。
        """
        try:
    ...
    def build_tune_tab(self):
        ...
        _btn_row=(len(items)+1)//2
        ttk.Button(sf,text='保存',command=self.save_settings).grid(row=_btn_row,column=0,pady=8); ttk.Button(sf,text='保存して再解析',command=self.save_settings_and_reanalyze).grid(row=_btn_row,column=1,pady=8)
...
"""
    ...
    def build_mindmap_tab(self):
        ...
        right_pane=ttk.PanedWindow(right,orient=tk.VERTICAL); right_pane.pack(fill=tk.BOTH,expand=True)

        # ── 工程・材料・工具ツリー（追加/編集/削除で自由に編集できる） ──
        tree_frame=ttk.Frame(right_pane); right_pane.add(tree_frame,weight=3)
        treebtn=ttk.Frame(tree_frame); treebtn.pack(fill=tk.X)
        ttk.Button(treebtn,text='追加',width=6,command=self.mm_add_node).pack(side=tk.LEFT,padx=1)
        ttk.Button(treebtn,text='編集',width=6,command=self.mm_edit_node).pack(side=tk.LEFT,padx=1)
        ttk.Button(treebtn,text='削除',width=6,command=self.mm_delete_node).pack(side=tk.LEFT,padx=1)
        ttk.Label(treebtn,text='※ノードを選んで追加すると、その子として追加されます').pack(side=tk.LEFT,padx=6)
        self.mm_tree=ttk.Treeview(tree_frame,columns=('type','qty','unit','minutes'),show='tree headings')
        for c,t in [('#0','ノード'),('type','type'),('qty','数量'),('unit','単位'),('minutes','分')]:
            self.mm_tree.heading(c,text=t)
        self.mm_tree.column('#0',width=180); self.mm_tree.column('type',width=70); self.mm_tree.column('qty',width=50); self.mm_tree.column('unit',width=45); self.mm_tree.column('minutes',width=45)
        self.mm_tree.pack(fill=tk.BOTH,expand=True)
        self.mm_tree.bind('<Double-1>',lambda e:self.mm_edit_node())
        self.mm_tree.bind('<Delete>',lambda e:self.mm_delete_node())
        self.mm_tree.bind('<Button-3>',self.show_mm_tree_context_menu)

        # ── JSONそのものを直接編集したい上級者向け ──
        json_frame=ttk.Frame(right_pane); right_pane.add(json_frame,weight=2)
        ttk.Label(json_frame,text='JSON直接編集（再描画ボタンでツリー/図に反映）').pack(anchor='w')
        self.mm_json_text=scrolledtext.ScrolledText(json_frame,height=7)
        self.mm_json_text.pack(fill=tk.BOTH,expand=True,pady=2)
        ttk.Button(json_frame,text='JSON再描画',command=self.load_mindmap_from_json_text).pack(anchor='e',pady=2)

        # ── AIチャット：相談しながら工程・材料・工具を組みなおす ──
        chat_frame=ttk.LabelFrame(right_pane,text='AIチャット（工程の相談・材料/工具の洗い出し）',padding=4)
        right_pane.add(chat_frame,weight=3)
        self.mm_chat_log=scrolledtext.ScrolledText(chat_frame,height=10,state='disabled',wrap='word')
        self.mm_chat_log.pack(fill=tk.BOTH,expand=True)
        self.mm_chat_log.tag_config('user',foreground='#0050c0')
        self.mm_chat_log.tag_config('ai',foreground='#207020')
        self.mm_chat_log.tag_config('sys',foreground='#999999')
        chat_in=ttk.Frame(chat_frame); chat_in.pack(fill=tk.X,pady=3)
        self.mm_chat_input=tk.StringVar()
        entry=ttk.Entry(chat_in,textvariable=self.mm_chat_input)
        entry.pack(side=tk.LEFT,fill=tk.X,expand=True,padx=2)
        entry.bind('<Return>',lambda e:self.mm_chat_send())
        ttk.Button(chat_in,text='送信',command=self.mm_chat_send).pack(side=tk.LEFT,padx=2)
        self.mm_apply_ai_btn=ttk.Button(chat_in,text='AI提案を適用',command=self.mm_apply_ai_proposal,state='disabled')
        self.mm_apply_ai_btn.pack(side=tk.LEFT,padx=2)
        self._mm_pending_ai_json=None
        self._mm_chat_history=[]
        self._mm_chat_append('sys','工程・材料・工具について、AIに相談しながら組みなおせます。'
                              '例：「ダウンライトを2台追加して」「スイッチ工程の手順をもっと細かく分けて」など。')

        self.current_mindmap=default_mindmap_json(self.mm_task_var.get(),1)
        self.render_mindmap()

    def show_mm_tree_context_menu(self,event):
        try:
            iid=self.mm_tree.identify_row(event.y)
            if iid: self.mm_tree.selection_set(iid)
            m=tk.Menu(self.root,tearoff=0)
            m.add_command(label='子ノードを追加',command=self.mm_add_node)
            m.add_command(label='編集',command=self.mm_edit_node)
            m.add_command(label='削除',command=self.mm_delete_node)
            m.tk_popup(event.x_root,event.y_root)
        finally:
            try: m.grab_release()
            except Exception: pass

    def _mm_selected_node_id(self):
        sel=self.mm_tree.selection()
        return sel[0] if sel else None

    def mm_add_node(self):
        """選択中ノードの子として新しいノード（工程/材料/工具など）を追加する。"""
        data=getattr(self,'current_mindmap',None) or default_mindmap_json(self.mm_task_var.get(),1)
        nodes=data.setdefault('mindmap_nodes',[])
        parent_id=self._mm_selected_node_id() or (nodes[0].get('id') if nodes else 'root')
        dlg=NodePropertyDialog(self.root)
        self.root.wait_window(dlg)
        if not dlg.result: return
        r=dlg.result
        existing_ids={n.get('id') for n in nodes}
        base=re.sub(r'[^a-zA-Z0-9_]','',r['type'] or 'node') or 'node'
        new_id=base; i=1
        while new_id in existing_ids:
            i+=1; new_id=f'{base}{i}'
        meta={}
        if r['type'] in ('material',): meta={'default_qty':r['qty'],'unit':r['unit']}
        elif r['type']=='step': meta={'order':len(nodes)+1,'minutes':r['qty']}
        elif r['type']=='tool': meta={}
        else: meta={'qty':r['qty'],'unit':r['unit'],'symbol':r['symbol']}
        nodes.append({'id':new_id,'text':r['text'],'parent':parent_id,'type':r['type'],'meta':meta})
        self.current_mindmap=data
        self.render_mindmap()
        try: self.mm_tree.selection_set(new_id); self.mm_tree.see(new_id)
        except Exception: pass
        self.status.config(text=f'ノード「{r["text"]}」を追加しました')

    def mm_edit_node(self):
        nid=self._mm_selected_node_id()
        if not nid:
            messagebox.showwarning('未選択','編集するノードを選択してください'); return
        data=getattr(self,'current_mindmap',None)
        if not data: return
        node=next((n for n in data.get('mindmap_nodes',[]) if n.get('id')==nid),None)
        if not node: return
        if node.get('parent') is None:
            messagebox.showinfo('対象外','ルートノードは工程名欄から編集してください'); return
        meta=node.get('meta') or {}
        init={'text':node.get('text',''),'type':node.get('type','material'),
              'qty':meta.get('default_qty',meta.get('minutes',meta.get('qty',1))),
              'unit':meta.get('unit','個'),'symbol':meta.get('symbol','')}
        dlg=NodePropertyDialog(self.root,init)
        self.root.wait_window(dlg)
        if not dlg.result: return
        r=dlg.result
        node['text']=r['text']; node['type']=r['type']
        if r['type']=='material': node['meta']={'default_qty':r['qty'],'unit':r['unit']}
        elif r['type']=='step': node['meta']={'order':meta.get('order',1),'minutes':r['qty']}
        elif r['type']=='tool': node['meta']={}
        else: node['meta']={'qty':r['qty'],'unit':r['unit'],'symbol':r['symbol']}
        self.render_mindmap()
        try: self.mm_tree.selection_set(nid)
        except Exception: pass
        self.status.config(text=f'ノード「{r["text"]}」を編集しました')

    def mm_delete_node(self):
        nid=self._mm_selected_node_id()
        if not nid:
            messagebox.showwarning('未選択','削除するノードを選択してください'); return
        data=getattr(self,'current_mindmap',None)
        if not data: return
        ...
        if target.get('parent') is None:
            messagebox.showwarning('削除不可','ルートノードは削除できません'); return
        # 子孫ノードも一括削除
        to_remove={nid}
        changed=True
        while changed:
            changed=False
            for n in nodes:
                if n.get('parent') in to_remove and n.get('id') not in to_remove:
                    to_remove.add(n.get('id')); changed=True
        if not messagebox.askyesno('確認',f'「{target.get("text","")}」と配下の{len(to_remove)-1}件を削除しますか？'):
            return
        data['mindmap_nodes']=[n for n in nodes if n.get('id') not in to_remove]
        self.current_mindmap=data
        self.render_mindmap()
        self.status.config(text=f'{len(to_remove)}件のノードを削除しました')

    def _mm_chat_append(self,role,text):
        try:
            self.mm_chat_log.configure(state='normal')
            prefix={'user':'あなた: ','ai':'AI: ','sys':''}.get(role,'')
            self.mm_chat_log.insert(tk.END,prefix+text+'\n\n',role)
            self.mm_chat_log.see(tk.END)
            self.mm_chat_log.configure(state='disabled')
        except Exception:
            pass

    def mm_chat_send(self):
        msg=self.mm_chat_input.get().strip()
        if not msg: return
        if getattr(self,'_mm_chat_busy',False):
            return
        self.mm_chat_input.set('')
        self._mm_chat_append('user',msg)
        self._mm_chat_history.append({'role':'user','text':msg})
        data=getattr(self,'current_mindmap',None) or default_mindmap_json(self.mm_task_var.get(),1)
        snap=self.snapshot_ai_settings() if hasattr(self,'snapshot_ai_settings') else {'provider':'ollama'}
        history_tail=self._mm_chat_history[-8:]
        self._mm_chat_busy=True
        self._mm_chat_append('sys','AIが検討しています…')
        def worker():
            convo='\n'.join(f"{'ユーザー' if h['role']=='user' else 'AI'}: {h['text']}" for h in history_tail)
            prompt=(
                'あなたは日本の電気設備工事の施工計画を手伝うアシスタントです。'
                '現在のToDoマインドマップ(JSON)を踏まえ、ユーザーの相談内容に日本語で答えてください。\n'
                '工程の組みなおし、必要な材料・工具の洗い出しや再確認について具体的に助言してください。\n'
                'もしJSON構造（mindmap_nodes）を更新すべきだと判断した場合のみ、回答の最後に\n'
                '"---JSON---" という行を書き、その次の行から更新後の完全なJSON1個だけを出力してください。\n'
                '構造は現在と同じ project_task, root_qty, total_estimated_minutes, mindmap_nodes(id,text,parent,type,meta) を保つこと。\n'
                'JSON更新が不要な場合は"---JSON---"以降を書かないでください。\n\n'
                f'現在のJSON:\n{json.dumps(data,ensure_ascii=False)}\n\n'
                f'これまでの会話:\n{convo}\n'
            )
            ans=str(UnifiedAIClient(snap).generate(prompt,timeout=60))
            explain=ans; proposed=None
            if '---JSON---' in ans:
                explain,_,jtxt=ans.partition('---JSON---')
                try:
                    proposed=sanitize_ai_json(jtxt)
                except Exception:
                    proposed=None
            return explain.strip(), proposed
        def done(res):
            explain,proposed=res
            self._mm_chat_busy=False
            # 「検討しています…」表示を消してから回答を出す
            try:
                self.mm_chat_log.configure(state='normal')
                self.mm_chat_log.delete('end-3l','end')
                self.mm_chat_log.configure(state='disabled')
            except Exception:
                pass
            self._mm_chat_append('ai',explain or '(応答なし)')
            self._mm_chat_history.append({'role':'ai','text':explain})
            if proposed:
                self._mm_pending_ai_json=proposed
                self.mm_apply_ai_btn.configure(state='normal')
                self._mm_chat_append('sys','AIが工程表の更新案を提案しました。内容を確認し「AI提案を適用」で反映できます。')
        def err(tb):
            self.log(tb); self._mm_chat_busy=False
            self._mm_chat_append('sys','AI応答でエラーが発生しました。AI接続設定を確認してください。')
        self.run_in_thread(worker,done,err)

    def mm_apply_ai_proposal(self):
        if not self._mm_pending_ai_json:
            return
        self.set_mindmap(self._mm_pending_ai_json)
        self._mm_pending_ai_json=None
        self.mm_apply_ai_btn.configure(state='disabled')
        self._mm_chat_append('sys','AIの提案をツリー・図に適用しました。')
        self.status.config(text='AI提案をマインドマップに適用しました')
    ...
    def separate_selected_detection(self):
        """
        重なっている/密集している/隣接している図面記号として誤認識され1つの
        検出候補にまとまってしまったものを、個々の記号候補へ分離する。
        """
        d=self.get_image_det(self.selected_det_id)
        if not d:
            messagebox.showwarning('未選択','分離する検出候補を選択してください')
            return
        if not self.image_pages or d.page<1 or d.page>len(self.image_pages):
            return
        img=self.image_pages[d.page-1]
        rules=self.db.get_image_rules()
        s=self.db.get_settings()
        bw=max(1,d.x2-d.x1); bh=max(1,d.y2-d.y1)
        # 「記号1個ぶんの想定サイズ」は、同一設備の他の（この塊ではない）検出候補の
        # 中央値から推定する。max_box_w/hはあくまで上限値なので、それをそのまま
        # 単体サイズとして使うと（上限を大きく設定している場合に）分割が効かない。
        same_eq=[x for x in self.image_dets if x.det_id!=d.det_id and x.equipment==d.equipment and x.enabled and x.w<bw*0.9 and x.h<bh*0.9]
        if same_eq:
            ws=sorted(x.w for x in same_eq); hs=sorted(x.h for x in same_eq)
            exp_w=ws[len(ws)//2]; exp_h=hs[len(hs)//2]
        else:
            cap_w=max(6,int(s.get('max_box_w','55'))); cap_h=max(6,int(s.get('max_box_h','55')))
            # 上限設定が塊自体より大きい（＝単体サイズの参考にならない）場合は、
            # 塊のサイズから「2〜3個くらいでは」と仮に見積もる
            exp_w=cap_w if cap_w < bw*0.9 else max(6,int(bw/2.2))
            exp_h=cap_h if cap_h < bh*0.9 else max(6,int(bh/2.2))
        try:
            new_dets=split_large_detection_smart(d,img,rules,s,exp_w=exp_w,exp_h=exp_h,force=True)
        except Exception:
            self.log(traceback.format_exc())
            new_dets=[d]
        if len(new_dets)<=1:
            messagebox.showinfo('分離結果','この候補はこれ以上分離できませんでした。\n手動で範囲追加するか、色ルールの結合距離を調整してください。')
            return
        self.image_dets=[x for x in self.image_dets if x.det_id!=d.det_id]
        for nd in new_dets:
            nd.memo=(nd.memo+' / manual_separate').strip()
        self.image_dets.extend(new_dets)
        for i,x in enumerate(sorted(self.image_dets,key=lambda z:(z.page,z.y1,z.x1)),1): x.det_id=i
        self.image_dets.sort(key=lambda z:z.det_id)
        self.selected_det_id=None
        self.refresh_image_view(); self.refresh_image_tables()
        self.status.config(text=f'検出候補ID={d.det_id} を {len(new_dets)} 個の記号に分離しました')
    ...
    def show_detection_context_menu(self,event):
        try:
            ...
            m.add_separator()
            m.add_command(label='選択候補を個別記号へ分離',command=self.separate_selected_detection)
    ...
    def finish_image_analysis(self,pages,dets):
        ...
        self.current_page=1
        self.selected_det_id=None
        # ページ読込直後は「全体が見えるフィット倍率」を初期ズームにする
        try:
            img0=pages[0] if pages else None
            if img0:
                vw,_vh=self._preview_canvas_size()
                self.preview_scale=max(self.zoom_min,min(1.0, vw/max(1,img0.width)))
        except Exception:
        ...
        try:
            self.preview_canvas.xview_moveto(0); self.preview_canvas.yview_moveto(0)
        except Exception:
            pass
        self.refresh_image_view()
        self.refresh_image_tables()
        if hasattr(self,'zoom_label_var'): self.zoom_label_var.set(f'{int(self.preview_scale*100)}%')
        self.status.config(text=f'画像解析完了: {len(dets)}件 / 学習反映 {learned_count}件 / ベクトル反映 {vector_count}件')
    ...
    def _schedule_preview_render(self, delay_ms=35):
        """スクロール/リサイズ/ズーム操作をまとめて間引いて再描画する（連続イベントでの重複描画防止）。"""
        try:
            if getattr(self, '_render_after_id', None):
                self.root.after_cancel(self._render_after_id)
        except Exception:
            pass
        self._render_after_id = self.root.after(delay_ms, self.refresh_image_view)

    def refresh_image_view(self):
        """
        v14: ビューポート（実際に見えている範囲）だけを元画像から切り出して描画する。
        巨大なPDF/DXFページでもズーム・スクロールのたびに画像全体を再描画しないため、
        フリーズせず滑らかに拡大縮小できる。検出枠等はベクタ（canvasアイテム）で
        オーバーレイするので、どの倍率でも文字や枠線が滲まずシャープに表示される。
        """
        if not self.image_pages:
            self.preview_canvas.delete('all'); return
        try:
            canvas = self.preview_canvas
            vw, vh = self._preview_canvas_size()
            vx0 = canvas.canvasx(0); vy0 = canvas.canvasy(0)
        except Exception:
            vw, vh, vx0, vy0 = 1120, 780, 0, 0
        self._preview_req = (self.current_page, self.selected_det_id, list(self.image_dets), float(self.preview_scale), vx0, vy0, vw, vh)
    ...
    def _preview_render_worker(self):
        try:
            while True:
                ...
                page, sel_id, dets, zoom, vx0, vy0, vw, vh = req
                ...
                full_w = img.width * zoom; full_h = img.height * zoom
                # キャンバス上に見えている範囲を画像座標へ変換して、その部分だけ切り出す
                cx0 = max(0.0, min(vx0, full_w)); cy0 = max(0.0, min(vy0, full_h))
                cx1 = max(0.0, min(vx0 + vw, full_w)); cy1 = max(0.0, min(vy0 + vh, full_h))
                if cx1 <= cx0 or cy1 <= cy0:
                    ix0=iy0=0; ix1=img.width; iy1=img.height
                else:
                    ix0 = int(cx0 / zoom); iy0 = int(cy0 / zoom)
                    ix1 = min(img.width, int(math.ceil(cx1 / zoom)) + 1); iy1 = min(img.height, int(math.ceil(cy1 / zoom)) + 1)
                ix0 = max(0, min(ix0, img.width - 1)); iy0 = max(0, min(iy0, img.height - 1))
                ix1 = max(ix0 + 1, min(ix1, img.width)); iy1 = max(iy0 + 1, min(iy1, img.height))
                try:
                    crop = img.crop((ix0, iy0, ix1, iy1))
                    tw = max(1, int(round((ix1 - ix0) * zoom))); th = max(1, int(round((iy1 - iy0) * zoom)))
                    if (tw, th) != crop.size:
                        resample = Image.Resampling.NEAREST if zoom >= 3 else Image.Resampling.BILINEAR
                        tile = crop.resize((tw, th), resample)
                    else:
                        tile = crop
                except Exception:
                    self.log(traceback.format_exc()); break
                page_d = [d for d in dets if d.page == page]
                # 見えている範囲＋余白に入る検出候補だけをオーバーレイ対象にする（大量検出でも軽い）
                margin = max(ix1 - ix0, iy1 - iy0) * 0.15
                visible_d = [d for d in page_d if not (d.x2 < ix0 - margin or d.x1 > ix1 + margin or d.y2 < iy0 - margin or d.y1 > iy1 + margin)]
                total = len(self.image_pages)

                def _up(tile=tile, zoom=zoom, ox=ix0 * zoom, oy=iy0 * zoom, page=page, total=total, sel_id=sel_id, visible_d=visible_d):
                    try:
                        if not self.root.winfo_exists():
                            return
                        self.preview_tk = ImageTk.PhotoImage(tile)
                        canvas = self.preview_canvas
                        canvas.delete('all')
                        canvas.create_image(ox, oy, image=self.preview_tk, anchor='nw', tags=('base_tile',))
                        cmap = {'purple': '#aa00ff', 'yellow': '#e6aa00', 'green': '#00b400', 'cyan': '#00aabe', 'blue': '#0050ff', 'red': '#ff0000', 'manual': '#ff00ff'}
                        for d in visible_d:
                            col = '#969696' if not d.enabled else cmap.get(d.color_name, '#ff00ff')
                            w = 3
                            if d.det_id == sel_id: col = '#ff0000'; w = 4
                            sx1, sy1, sx2, sy2 = d.x1 * zoom, d.y1 * zoom, d.x2 * zoom, d.y2 * zoom
                            canvas.create_rectangle(sx1, sy1, sx2, sy2, outline=col, width=w, tags=('det_box', f'det_{d.det_id}'))
                            canvas.create_text(sx1, max(0, sy1 - 12), text=str(d.det_id), fill=col, anchor='w', tags=('det_label',))
                        try:
                            for it in getattr(self, 'manual_symbol_items', []):
                                if int(it.get('page', 0)) == page:
                                    sx = float(it.get('x', 0)) * zoom
                                    sy = float(it.get('y', 0)) * zoom
                                    draw_symbol_glyph_on_canvas(canvas, sx, sy, it.get('name', ''), scale=zoom, tag='manual_symbol')
                        except Exception:
                            pass
                        try:
                            for row in getattr(self, 'manual_cable_lines', []):
                                ...
                                if pg == self.current_page:
                                    ...
                                    canvas.create_line(sx0, sy0, sx1, sy1, fill='orange', width=3)
                                    canvas.create_text((sx0 + sx1) / 2, (sy0 + sy1) / 2, text=f'{cable} {length_m:.1f}m', fill='orange', anchor='s')
                        except Exception:
                            pass
                        try: self.draw_torii_on_canvas()
                        except Exception: pass
                        canvas.config(scrollregion=(0, 0, img.width * zoom, img.height * zoom))
                        self.page_label.set(f'page {page}/{total}  ズーム {int(zoom*100)}%')
                        if hasattr(self, 'zoom_label_var'): self.zoom_label_var.set(f'{int(zoom*100)}%')
                    except Exception:
                        self.log(traceback.format_exc())
                self.ui(_up)
    ...
    def _wheel_dir(self,event):
        """WindowsのMouseWheel(delta)とLinux X11のButton-4/5の両方に対応した方向判定。"""
    ...
    def on_preview_mousewheel(self,event):
        """マウスホイール = カーソル位置を中心に図面を拡大縮小する。"""
        try:
            factor=1.15 if self._wheel_dir(event)>0 else (1/1.15)
            self.zoom_at_point(event.x,event.y,factor)
        except Exception:
            self.log(traceback.format_exc())

    def on_preview_shift_mousewheel(self,event):
        """Shift+ホイール = 水平スクロール（従来通り）。"""
        try:
            self.preview_canvas.xview_scroll(-1*self._wheel_dir(event), 'units')
            self._schedule_preview_render()
        except Exception:
            pass

    def on_preview_ctrl_mousewheel(self,event):
        """Ctrl+ホイール = 垂直スクロール（ホイール本体はズームに割当てたための代替）。"""
        try:
            self.preview_canvas.yview_scroll(-1*self._wheel_dir(event), 'units')
            self._schedule_preview_render()
        except Exception:
            pass

    def zoom_at_point(self, ex, ey, factor, redraw=True):
        """キャンバス座標(ex,ey)の指す図面上の点を画面上に固定したままズームする。"""
        if not self.image_pages: return
        img=self.image_pages[self.current_page-1] if 0<=self.current_page-1<len(self.image_pages) else None
        if img is None: return
        old_zoom=float(self.preview_scale)
        new_zoom=max(self.zoom_min,min(self.zoom_max,old_zoom*factor))
        if abs(new_zoom-old_zoom)<1e-6: return
        canvas=self.preview_canvas
        try:
            cx=canvas.canvasx(ex); cy=canvas.canvasy(ey)
        except Exception:
            cx=ex; cy=ey
        img_x=cx/max(0.0001,old_zoom); img_y=cy/max(0.0001,old_zoom)
        self.preview_scale=new_zoom
        full_w=img.width*new_zoom; full_h=img.height*new_zoom
        try: canvas.configure(scrollregion=(0,0,full_w,full_h))
        except Exception: pass
        new_cx=img_x*new_zoom; new_cy=img_y*new_zoom
        vw=max(1,canvas.winfo_width()); vh=max(1,canvas.winfo_height())
        target_left=max(0.0,new_cx-ex); target_top=max(0.0,new_cy-ey)
        try:
            if full_w>vw: canvas.xview_moveto(max(0.0,min(1.0,target_left/max(1.0,full_w-vw))))
            else: canvas.xview_moveto(0.0)
            if full_h>vh: canvas.yview_moveto(max(0.0,min(1.0,target_top/max(1.0,full_h-vh))))
            else: canvas.yview_moveto(0.0)
        except Exception:
            pass
        if hasattr(self,'zoom_label_var'): self.zoom_label_var.set(f'{int(new_zoom*100)}%')
        if redraw: self._schedule_preview_render(delay_ms=10)

    def zoom_step(self, factor):
        """ツールバーの＋／－ボタン用：キャンバス中央を基準にズームする。"""
        try:
            cw,ch=self._preview_canvas_size()
            self.zoom_at_point(cw/2, ch/2, factor)
        except Exception:
            self.log(traceback.format_exc())
    ...
    def zoom_fit(self):
        ...
        img=self.image_pages[self.current_page-1]
        vw,vh=self._preview_canvas_size()
        fit=min(vw/max(1,img.width), vh/max(1,img.height))
        self.preview_scale=max(self.zoom_min,min(self.zoom_max,fit))
        try:
            self.preview_canvas.xview_moveto(0); self.preview_canvas.yview_moveto(0)
        except Exception:
            pass
        if hasattr(self,'zoom_label_var'): self.zoom_label_var.set(f'{int(self.preview_scale*100)}%')
        self._schedule_preview_render(delay_ms=10)

    def zoom_100(self):
        if not self.image_pages: return
        self.preview_scale=1.0
        if hasattr(self,'zoom_label_var'): self.zoom_label_var.set('100%')
        self._schedule_preview_render(delay_ms=10)
