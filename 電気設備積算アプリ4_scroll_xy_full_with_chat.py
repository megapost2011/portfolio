...
#  AIチャット設定
# -----------------------------
AI_CONFIG_FILENAME = ".ai_chat_config.json"
...
# -----------------------------
#  Scrollable UI helper (縦＋横スクロール)
#  - Notebookタブ内をスクロール可能にする
# -----------------------------
class ScrollableArea:
    """
    ttk.Frame(親) の中に Canvas + 縦/横スクロールバー を作り、
    その中に ttk.Frame(inner) を配置して返す。
    既存コードの pack/grid をほぼそのまま流用できるようにする。
    """
    def __init__(self, parent):
        ...
        # update inner height/width only when canvas size changes (do NOT force inner width -> keep horizontal scroll alive)
        self.canvas.bind("<Configure>", self._on_canvas_configure)

        # mouse wheel (PC向け。Pydroidではスクロールバー操作が確実)
    ...
    def _on_canvas_configure(self, event=None):
        # keep the window at least as wide/high as canvas so small UIs don't look odd,
        # but allow it to grow wider than canvas -> horizontal scroll works.
        try:
            inner_req_w = self.inner.winfo_reqwidth()
            inner_req_h = self.inner.winfo_reqheight()
            w = max(event.width, inner_req_w)
            h = max(event.height, inner_req_h)
    ...
    def _bind_mousewheel(self, widget):
        ...
        def _on_mousewheel(event):
            # vertical
            if getattr(event, "state", 0) & 0x1:  # Shift -> horizontal
...
class AutoEstimationSystem:
    ...
    def create_widgets(self):
        ...
        tab_est_scroll = ScrollableArea(tab_est)
        self._scroll_tab_est = tab_est_scroll
        self._build_estimation_tab(tab_est_scroll.frame())

        tab_price = ttk.Frame(self.notebook, padding=6)
        self.notebook.add(tab_price, text="  単価マスター  ")
        tab_price_scroll = ScrollableArea(tab_price)
        self._scroll_tab_price = tab_price_scroll
        self._build_price_tab(tab_price_scroll.frame())

        tab_sym = ttk.Frame(self.notebook, padding=6)
        self.notebook.add(tab_sym, text="  記号パターン  ")
        tab_sym_scroll = ScrollableArea(tab_sym)
        self._scroll_tab_sym = tab_sym_scroll
        self._build_symbol_tab(tab_sym_scroll.frame())

        tab_cad = ttk.Frame(self.notebook, padding=6)
        self.notebook.add(tab_cad, text="  CADライブラリ  ")
        tab_cad_scroll = ScrollableArea(tab_cad)
        self._scroll_tab_cad = tab_cad_scroll
        self._build_cad_tab(tab_cad_scroll.frame())

        tab_ai = ttk.Frame(self.notebook, padding=6)
        self.notebook.add(tab_ai, text="  AIチャット  ")
        tab_ai_scroll = ScrollableArea(tab_ai)
        self._scroll_tab_ai = tab_ai_scroll
        self._build_ai_chat_tab(tab_ai_scroll.frame())
    ...
    # ============================================================
    #  AIチャット（Claude/Anthropic想定）
    # ============================================================
    def _ai_get_config_path(self):
        # Android(Pydroid)なら Download に置く / それ以外はスクリプト同階層
        candidates = [
        ...
        for d in candidates:
            try:
                if d and os.path.isdir(d):
                    return os.path.join(d, AI_CONFIG_FILENAME)
            except Exception:
                pass
        return os.path.join(os.getcwd(), AI_CONFIG_FILENAME)

    def _ai_load_config(self):
        path = self._ai_get_config_path()
    ...
    def _build_ai_chat_tab(self, parent):
        ...
        self.ai_cfg = self._ai_load_config()
        self.ai_api_key = self.ai_cfg.get("api_key", "")
        self.ai_model = tk.StringVar(value=self.ai_cfg.get("model", DEFAULT_CLAUDE_MODEL))

        hdr = ttk.Frame(parent)
        hdr.pack(fill=tk.X, pady=(2,6))
        ttk.Label(hdr, text="AIチャット（積算サポート）", font=("Arial", 12, "bold")).pack(side=tk.LEFT)
        self.ai_status = ttk.Label(hdr, text="起動中...", foreground="gray")
        self.ai_status.pack(side=tk.RIGHT)

        tools = ttk.Frame(parent)
        tools.pack(fill=tk.X, pady=(0,6))

        ttk.Label(tools, text="モデル:").pack(side=tk.LEFT, padx=(0,4))
        ...
        self.ai_model_combo.pack(side=tk.LEFT, padx=(0,8))

        ttk.Button(tools, text="APIキー設定", command=self._ai_set_api_key).pack(side=tk.LEFT, padx=4)
        ttk.Button(tools, text="接続テスト", command=self._ai_test).pack(side=tk.LEFT, padx=4)
        ttk.Button(tools, text="会話リセット", command=self._ai_clear).pack(side=tk.LEFT, padx=4)
        ...
        self.ai_chat = scrolledtext.ScrolledText(
            ...
            font=("", 11), spacing1=3, spacing3=3,
            height=18
        )
        self.ai_chat.pack(fill=tk.BOTH, expand=True)

        self.ai_chat.tag_configure("user", foreground="#0066cc", font=("", 11, "bold"))
        self.ai_chat.tag_configure("ai", foreground="#333333")
        self.ai_chat.tag_configure("system", foreground="#888888", font=("", 9, "italic"))
        self.ai_chat.tag_configure("error", foreground="#cc0000")

        # 入力欄
        inf = ttk.Frame(parent)
        inf.pack(fill=tk.X, pady=(8,4))

        self.ai_input = tk.StringVar()
        self.ai_entry = ttk.Entry(inf, textvariable=self.ai_input, font=("", 12))
        self.ai_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0,4))
        self.ai_entry.bind("<Return>", lambda e: self._ai_send())

        # Pydroid日本語入力回避（貼り付け）
        add_paste_button(inf, self.ai_entry)

        self.ai_send_btn = ttk.Button(inf, text="送信", command=self._ai_send, width=6)
        self.ai_send_btn.pack(side=tk.LEFT, padx=2)

        hint = ttk.Label(parent, text="※ 図面の内容/単価/記号の解釈などを自然文で相談できます。", foreground="gray")
        hint.pack(anchor="w", pady=(2,0))

        # 初期メッセージ
        if self.requests is None:
            self._ai_append("error", "requests が未導入です。ツール → requestsインストール を実行してください。")
            self.ai_status.config(text="requests未導入")
        elif not self.ai_api_key:
            self._ai_append("system", "APIキーが未設定です。［APIキー設定］で貼り付けてください。")
            self.ai_status.config(text="APIキー未設定")
        else:
            self._ai_append("system", "準備完了！質問を入力してください。")
            self.ai_status.config(text="準備完了")

        self.root.after(120, self._ai_poll)

    def _ai_append(self, role, text):
        self.ai_chat.configure(state=tk.NORMAL)
        if role == "user":
            self.ai_chat.insert(tk.END, "あなた: ", "user")
        elif role == "ai":
            self.ai_chat.insert(tk.END, "AI: ", "ai")
        elif role == "error":
            self.ai_chat.insert(tk.END, "エラー: ", "error")
        else:
            self.ai_chat.insert(tk.END, "システム: ", "system")
        self.ai_chat.insert(tk.END, text + "\n\n", role if role in ("user","ai","system","error") else "system")
        self.ai_chat.configure(state=tk.DISABLED)
        self.ai_chat.see(tk.END)

    def _ai_set_api_key(self):
        win = tk.Toplevel(self.root)
        win.title("AI APIキー設定")
        win.geometry("520x240")
        frm = ttk.Frame(win, padding=10)
        frm.pack(fill=tk.BOTH, expand=True)

        ttk.Label(frm, text="APIキー（Anthropic / Claude）を貼り付け：").pack(anchor="w")
        key_var = tk.StringVar(value=self.ai_api_key)
        ent = ttk.Entry(frm, textvariable=key_var, show="*", font=("", 11))
        ent.pack(fill=tk.X, pady=6)
        add_paste_button(frm, ent)

        ttk.Label(frm, text="モデル（任意）:").pack(anchor="w", pady=(8,0))
        model_var = tk.StringVar(value=self.ai_model.get())
        cb = ttk.Combobox(frm, textvariable=model_var, values=list(self.ai_model_combo["values"]), state="readonly")
        cb.pack(fill=tk.X, pady=6)

        def save():
            self.ai_api_key = key_var.get().strip()
            self.ai_model.set(model_var.get().strip() or DEFAULT_CLAUDE_MODEL)
            self.ai_cfg["api_key"] = self.ai_api_key
            self.ai_cfg["model"] = self.ai_model.get()
            self._ai_save_config(self.ai_cfg)
            if self.ai_api_key:
                self.ai_status.config(text="準備完了")
                self._ai_append("system", "APIキーを保存しました。")
            else:
                self.ai_status.config(text="APIキー未設定")
                self._ai_append("system", "APIキーが空です。")
            win.destroy()

        btns = ttk.Frame(frm)
        btns.pack(fill=tk.X, pady=10)
        ttk.Button(btns, text="保存", command=save).pack(side=tk.LEFT)
    ...
    def _ai_clear(self):
        ...
        try:
            ...
            pass
        self._ai_append("system", "会話をリセットしました。")

    def _ai_test(self):
        if self.requests is None:
            messagebox.showerror("エラー", "requestsが未導入です。ツール→requestsインストール を実行してください。")
            return
        if not self.ai_api_key:
            messagebox.showwarning("未設定", "APIキーが未設定です。［APIキー設定］で入力してください。")
            return
        self._ai_append("system", "接続テスト中...")
        self.ai_status.config(text="接続テスト中...")
        def worker():
            try:
                r = self._ai_call("こんにちは。接続テストです。短く返事してください。")
                self.ai_q.put(("ai", r))
                self.ai_q.put(("status", "準備完了"))
            except Exception as e:
                self.ai_q.put(("error", str(e)))
                self.ai_q.put(("status", "エラー"))
        threading.Thread(target=worker, daemon=True).start()
    ...
    def _ai_send(self):
        ...
        if not msg:
            return
        if self.requests is None:
            self._ai_append("error", "requestsが未導入です。ツール→requestsインストール を実行してください。")
            return
        if not self.ai_api_key:
            self._ai_append("error", "APIキーが未設定です。［APIキー設定］で貼り付けてください。")
            return

        self.ai_input.set("")
        self._ai_append("user", msg)
        self.ai_status.config(text="送信中...")

        def worker():
            try:
                reply = self._ai_call(msg)
                self.ai_q.put(("ai", reply))
                self.ai_q.put(("status", "準備完了"))
            except Exception as e:
                self.ai_q.put(("error", str(e)))
                self.ai_q.put(("status", "エラー"))
    ...
    def _ai_call(self, user_text):
        # 会話履歴（最大20往復程度に圧縮）
        self.ai_conversation.append({"role": "user", "content": user_text})
        if len(self.ai_conversation) > 40:
            self.ai_conversation = self.ai_conversation[-40:]

        sys_prompt = (
            "あなたは電気設備積算のアシスタントです。\n"
            "- PDF/DXF/画像の図面から読み取れる情報の整理\n"
            "- 単価・数量・記号の解釈\n"
            "- 積算手順の提案\n"
            "に強いです。\n"
            "不明点は推測ではなく確認事項を列挙し、できるだけ具体的に答えてください。"
        )

        payload = {
            "model": (self.ai_model.get() or DEFAULT_CLAUDE_MODEL),
            "max_tokens": 800,
            "system": sys_prompt,
        ...
        resp = self.requests.post(CLAUDE_API_URL, headers=headers, json=payload, timeout=60)
        if resp.status_code != 200:
            raise RuntimeError(f"HTTP {resp.status_code}: {resp.text[:500]}")
        data = resp.json()
        # Anthropic messages API: content -> list of blocks
        ...
        if not text_out:
            text_out = "(応答が空でした)"
        # 会話履歴にAI応答を追加
        self.ai_conversation.append({"role": "assistant", "content": text_out})
        return text_out

    def _ai_poll(self):
        try:
            while True:
                item = self.ai_q.get_nowait()
                kind, payload = item[0], item[1]
                if kind == "ai":
                    self._ai_append("ai", payload)
                elif kind == "error":
                    self._ai_append("error", payload)
                elif kind == "status":
                    self.ai_status.config(text=payload)
        except queue.Empty:
            pass
        except Exception as e:
            # 何かあっても落とさない
            try:
                self._ai_append("error", str(e))
            except Exception:
                pass
        finally:
            self.root.after(120, self._ai_poll)
